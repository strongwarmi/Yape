...
module.exports = (router, db) => {
        ...
        if (userId) {
            db.get(userId, (err, record) => {
                if (err) {
                    ...
                    return res.json({
                        success: false,
                        message: "Hubo error al obtener el registro",
                        data: null
                    });
                }
                const gen = rn.generator({
        ...
        if (phone && name && email && password) {
            db.get(phone, (err, record) => {
                ...
                if (record.terms && record.code) {
                    ...
                    return res.json({
                        success: false,
                        message: "Terminos no aceptados y codigo no generado"
                    });
                }
            });
        } else {
    ...
    router.post('/registerCard', (req, res) => {
        ...
        if (userId && cardNumber && cardYear && cardMonth && cardPassword) {
            db.get(userId, (err, record) => {
                if (err) {
                    if (err.notFound) return res.json({
                        success: false,
                    ...
                    return res.json({
                        success: false,
                        message: "Hubo un error obteniendo el usuario",
                        data: false
                    });
                }
                const user = Object.assign({}, record, {
                ...
                db.put(userId, user, (err) => {
                    if (err) {
                        return res.json({
                            success: false,
                            message: "Hubo un problema al agregar la tarjeta",
                            data: null
                        });
                    }
                });
